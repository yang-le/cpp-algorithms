...
TEST(BinaryTreeNodeTest, replace_child) {
	...
	EXPECT_TRUE(rootNode->replaceChild(rootNode->right_.get(), rootNode->right_->right_));
	...
	EXPECT_FALSE(rootNode->replaceChild(rootNode->right_.get(), rootNode->right_->right_));
	EXPECT_EQ(rootNode->traverseInOrder(), std::vector<int>({ 1, 2, 5 }));
	...
	EXPECT_FALSE(rootNode->replaceChild(std::make_shared<BinaryTreeNode<int>>().get(), std::make_shared<BinaryTreeNode<int>>()));
}
...
TEST(BinaryTreeNodeTest, right_uncle) {
	auto grandParent = std::make_shared<BinaryTreeNode<std::string>>("grand-parent");
