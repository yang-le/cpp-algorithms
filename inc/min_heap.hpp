...
class MinHeap
{
  public:
	...
	MinHeap *remove(const T &item, Comparator<T> comparator = nullptr)
	{
		...
		for (int iteration = 0; iteration < numberOfItemsToRemove; ++iteration)
		{
			...
			else
			{
				...
				if (hasLeftChild(indexToRemove) && (!hasParent(indexToRemove) || comparator_.lessThan(parent(indexToRemove), container_[indexToRemove])))
				{
	...
	std::vector<int> find(const T &item, Comparator<T> custom_comparator = nullptr)
	{
	...
	std::string toString()
	{
		std::string ret = std::accumulate(container_.begin(), container_.end(), std::string(), [](const std::string &a, const T &b) {
			std::stringstream ss;
			ss << a << "," << b;
			return ss.str();
		});
  ...
  private:
	...
	void heapifyUp(int customStartIndex = -1)
	{
		...
		auto currentIndex = (customStartIndex < 0) ? container_.size() - 1 : customStartIndex;

		while (hasParent(currentIndex) && comparator_.lessThan(container_[currentIndex], parent(currentIndex)))
		{
	...
	void heapifyDown(int customStartIndex = -1)
	{
		...
		{
			if (hasRightChild(currentIndex) && comparator_.lessThan(rightChild(currentIndex), leftChild(currentIndex)))
			{
			...
			if (comparator_.lessThan(container_[currentIndex], container_[nextIndex]))
			{
				break;
			}
