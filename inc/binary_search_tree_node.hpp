...
class BinarySearchTreeNode : public BinaryTreeNode<T>
{
  public:
	BinarySearchTreeNode(typename Comparator<T>::compare_func_t compare = nullptr)
		: BinaryTreeNode<T>(), compare_(compare), value_comparator_(Comparator<T>(compare)) {}

	BinarySearchTreeNode(const T &value, typename Comparator<T>::compare_func_t compare = nullptr)
		: BinaryTreeNode<T>(value), compare_(compare), value_comparator_(Comparator<T>(compare)) {}
	...
	BinarySearchTreeNode *insert(const T &value)
	{
		...
		if (this->value_comparator_.lessThan(value, this->value_))
		{
			...
			if (left)
			{
				return std::static_pointer_cast<BinarySearchTreeNode>(left)->insert(value);
			}
		...
		if (this->value_comparator_.greaterThan(value, this->value_))
		{
			...
			if (right)
			{
				return std::static_pointer_cast<BinarySearchTreeNode>(right)->insert(value);
			}
	...
	bool remove(const T &value)
	{
		...
		else if (nodeToRemove->left_ && nodeToRemove->right_)
		{
			...
			auto nextBiggerNode = std::static_pointer_cast<BinarySearchTreeNode>(nodeToRemove->right_)->findMin();
			if (!this->node_comparator_.equal(nextBiggerNode, nodeToRemove->right_.get()))
			{
		...
		else
		{
			...
			auto childNode = nodeToRemove->left_ ? nodeToRemove->left_ : nodeToRemove->right_;
	...
	BinarySearchTreeNode *findMin()
	{
		...
		return std::static_pointer_cast<BinarySearchTreeNode>(this->left_)->findMin();
	}
